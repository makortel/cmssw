    ...
    for(size_t il=0; il!=size; ++il) {
      ...
      for(auto hitIndex: foundNodes) {
        ...
        if(useBendingCorrection) {
          ...
          std::vector<float> r(4), z(4), errZ(4);
          ...
          for (int i=0; i< 4; ++i) {
            ...
            r[i] += pixelrecoutilities::LongitudinalBendingCorrection(pt,es)(r[i]);
            z[i] = point.z()-region.origin().z();
            errZ[i] =  (barrels[i]) ? sqrt(error.czz()) : sqrt( error.rerr(point) )*simpleCot;
          }
