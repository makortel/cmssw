...
public:
  ...
  void start(std::string_view name, bool nameIsString) final {
    MonitorAdaptor::startOnThread(name, pattern_, config_);
  }
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& iDesc) {
    ...
    ps.addUntracked<bool>("deallocationReport", true)->setComment("Whether to produce a report on deallocations. On deep stack traces this can take time, so turning it off could speed up if the deallocation report is not needed");
    ps.addUntracked<bool>("churnReport", true)->setComment("Whether to produce reports on memory churn. On deep stack traces this can take time, so turning it off could speed up if the deallocation report is not needed");
    iDesc.addDefault(ps);
  }
