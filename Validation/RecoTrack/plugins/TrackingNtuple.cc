...
void TrackingNtuple::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const TrackingParticleRefKeySet *tpKeysPtr = nullptr;
  ...
  if(includeAllHits_) {
    //pixel hits
    fillPixelHits(iEvent, clusterToTPMap, tpKeysPtr, *simHitsTPAssoc, *theTTRHBuilder, tTopo, tpHitList, hitProductIds);

    //strip hits
    fillStripRphiStereoHits(iEvent, clusterToTPMap, tpKeysPtr, *simHitsTPAssoc, *theTTRHBuilder, tTopo, tpHitList, hitProductIds);
  ...
  for (auto it = pixelHits->begin(); it!=pixelHits->end(); it++ ) {
    ...
    for (auto hit = it->begin(); hit!=it->end(); hit++ ) {
      ...
      SimHitData simHitData = matchCluster(hit->firstClusterRef(), hitId, key, ttrh,
                                           clusterToTPMap, trackingParticleKeys, simHitsTPAssoc, tpHitList, HitPixel);
  ...
  auto fill = [&](const SiStripRecHit2DCollection& hits, const char *name, bool isStereo) {
    for(const auto& detset: hits) {
      ...
      for(const auto& hit: detset) {
        ...
        const int lay = tTopo.layer(hitId);
        SimHitData simHitData = matchCluster(hit.firstClusterRef(), hitId, key, ttrh,
                                             clusterToTPMap, trackingParticleKeys, simHitsTPAssoc, tpHitList, HitStrip);
