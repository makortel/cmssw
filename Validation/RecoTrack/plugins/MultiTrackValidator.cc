...
MultiTrackValidator::MultiTrackValidator(const edm::ParameterSet& pset):MultiTrackValidatorBase(pset,consumesCollector()){
  ...
  if(UseAssociators) {
    for (auto const& associatorLabel : associators) {
      associatorTokens.push_back(consumes<reco::TrackToTrackingParticleAssociator>(associatorLabel.src));
    }
  } else {   
    for (auto const& associatorLabel : associators) {
      associatormapStRs.push_back(consumes<reco::SimToRecoCollection>(associatorLabel.src));
      associatormapRtSs.push_back(consumes<reco::RecoToSimCollection>(associatorLabel.src));
    }
  }
}
