...
MultiTrackValidatorGenPs::MultiTrackValidatorGenPs(const edm::ParameterSet& pset):MultiTrackValidator(pset){
  ...
  if(UseAssociators) {
    for(auto const& associatorLabel: associators) {
      if( associatorLabel.src.label() == kTrackAssociatorByChi2) {
        label_gen_associator = consumes<reco::TrackToGenParticleAssociator>(associatorLabel.src);
        break;
      }
    }
  }
    ...
    for (auto const& associatorLabel : associators) {
      associatormapGtR = consumes<reco::GenToRecoCollection>(associatorLabel.src);
      associatormapRtG = consumes<reco::RecoToGenCollection>(associatorLabel.src);
      break;
    }
  }
}
