...
RunManagerMTMaster::~RunManagerMTMaster() 
{ 
...
void RunManagerMTMaster::initG4(const edm::EventSetup & es)
{
  ...
  edm::LogInfo("SimG4CoreApplication") 
    << " RunManagerMTMaster: Sensitive Detector "
  ...
  for (unsigned it=0; it<m_G4Commands.size(); it++) {
    edm::LogInfo("SimG4CoreApplication") << "RunManagerMTMaster:: Requests UI: "
                                         << m_G4Commands[it];
...
void RunManagerMTMaster::stopG4()
{
  //std::cout << "RunManagerMTMaster::stopG4" << std::endl;
  G4StateManager::GetStateManager()->SetNewState(G4State_Quit);
  if (!m_runTerminated) { terminateRun(); }
}

void RunManagerMTMaster::produce(edm::Event& inpevt, const edm::EventSetup & es)
{
  ...
  if (m_currentEvent->GetNumberOfPrimaryVertex()==0) {
    edm::LogError("SimG4CoreApplication") 
      << " RunManagerMTMaster::produce event " << inpevt.id().event()
  ...
  edm::LogInfo("SimG4CoreApplication") 
    << " RunManagerMTMaster: saved : Event  " << inpevt.id().event() 
...
G4Event * RunManagerMTMaster::generateEvent(edm::Event & inpevt)
{                       
...
void RunManagerMTMaster::abortEvent()
{
...
void RunManagerMTMaster::initializeUserActions()
{
  ...
  if (m_generator!=0) {
    ...
    edm::LogWarning("SimG4CoreApplication") << " RunManagerMTMaster: WARNING : "
					    << "No generator; initialized "
					    << "only RunAction!";
  }
}

void RunManagerMTMaster::initializeRun()
{
...
void RunManagerMTMaster::terminateRun()
{
...
void RunManagerMTMaster::abortRun(bool softAbort)
{
  m_runAborted = false;
...
void RunManagerMTMaster::resetGenParticleId( edm::Event& inpevt ) 
{
...
SimTrackManager* RunManagerMTMaster::GetSimTrackManager()
{
  return m_trackManager.get();
}

void  RunManagerMTMaster::Connect(RunAction* runAction)
{
...
void  RunManagerMTMaster::Connect(EventAction* eventAction)
{
...
void  RunManagerMTMaster::Connect(TrackingAction* trackingAction)
{
...
void  RunManagerMTMaster::Connect(SteppingAction* steppingAction)
{
  steppingAction->m_g4StepSignal.connect(m_registry.g4StepSignal_);
}

void RunManagerMTMaster::DumpMagneticField(const G4Field* field) const
{
  std::ofstream fout(m_FieldFile.c_str(), std::ios::out);
  if(fout.fail()){
    edm::LogWarning("SimG4CoreApplication") 
      << " RunManagerMTMaster WARNING : "
