  ...
  counterEvt_(0)
{
  ...
  for(const edm::ParameterSet& pset: monitorSetup) {
    ...
    if(configtype == "L1") {
      try {
        l1Plotters_.emplace_back(new HLTTauDQML1Plotter(pset, nPtBins, nEtaBins, nPhiBins, etMax, doRefAnalysis_, L1MatchDr_, dqmBaseFolder_));
      } catch(cms::Exception& e) {
        edm::LogWarning("HLTTauDQMOfflineSource") << e.what() << std::endl;
        continue;
      }
      ...
      try {
        pathPlotters2_.emplace_back(new HLTTauDQMPathPlotter2(pset, doRefAnalysis_, dqmBaseFolder_, hltProcessName_, nPtBins, nEtaBins, nPhiBins, HLTMatchDr_));
      } catch ( cms::Exception &e ) {
    ...
    if (counterEvt_ > prescaleEvt_) {
        ...
        if (doRefAnalysis_) {
          for(RefObject& refObj: refObjects_) {
            ...
            iEvent.getByLabel(refObj.src, collHandle);
