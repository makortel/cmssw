...
ConversionTrackCandidateProducer::ConversionTrackCandidateProducer(const edm::ParameterSet& config) : 
  conf_(config), 
  theTrajectoryBuilder_(edm::Service<edm::service::PluginFactoryService>()->create<BaseCkfTrajectoryBuilderFactory>(config.getParameter<std::string>("TrajectoryBuilder"))),
