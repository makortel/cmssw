...
#include "CommonTools/Utils/interface/associationMapFilterKeys.h"
#include "DataFormats/Common/interface/AssociationMap.h"

#include <cppunit/extensions/HelperMacros.h>

class testAssociationMapFilterKeys : public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(testAssociationMapFilterKeys);
...
CPPUNIT_TEST_SUITE_REGISTRATION( testAssociationMapFilterKeys );
...
void testAssociationMapFilterKeys::checkOneToOne() {
  ...
  // vector of Refs
  std::vector<edm::Ref<CKey>> keep{edm::Ref<CKey>(&keys, 0), edm::Ref<CKey>(&keys, 2)};
  Assoc filtered = associationMapFilterKeys(map, keep);
  CPPUNIT_ASSERT( filtered.size() == 2 );
  CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 0)) != filtered.end() );
  CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 0)] == edm::Ref<CVal>(&values, 0) );
  ...
  CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 2)] == edm::Ref<CVal>(&values, 2) );

  // RefVector
  edm::RefVector<CKey> keep2;
  keep2.push_back(edm::Ref<CKey>(&keys, 1));
  filtered = associationMapFilterValues(map, keep2);
  CPPUNIT_ASSERT( filtered.size() == 1 );
  CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 0)) == filtered.end() );
  CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 1)) != filtered.end() );
  CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 1)] == edm::Ref<CVal>(&values, 1) );
  CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 2)) == filtered.end() );

  // Mostly check that it compiles
  edm::View<CKey> keepView;
  filtered = associationMapFilterValues(map, keepView);
  CPPUNIT_ASSERT( filtered.size() == 0 );
}

void testAssociationMapFilterKeys::checkOneToMany() {
  // simple data
  {
    typedef std::vector<int> CKey;
    typedef std::vector<double> CVal;
    ...
    CPPUNIT_ASSERT( filtered.size() == 2 );
    CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 0)) != filtered.end() );
    CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 0)].size() == 1 );
    CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 0)][0] == edm::Ref<CVal>(&values, 0) );
    CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 1)) == filtered.end() );
    CPPUNIT_ASSERT( filtered.find(edm::Ref<CKey>(&keys, 2)) != filtered.end() );

    std::cout << "\n"
              << filtered[edm::Ref<CKey>(&keys, 2)].size()
              << "\n"
              << filtered[edm::Ref<CKey>(&keys, 2)][0].key()
              << "\n\n";


    CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 2)].size() == 2 );
    CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 2)][0] == edm::Ref<CVal>(&values, 2) );
    CPPUNIT_ASSERT( filtered[edm::Ref<CKey>(&keys, 2)][1] == edm::Ref<CVal>(&values, 3) );
  }
#ifdef FOO
...
void testAssociationMapFilterKeys::checkOneToManyQuality() {
#ifdef FOO
  // simple data
  {
    typedef std::vector<int> CKey;
