...
void RawToDigi_wrapper(
    ...
    bool useQualityInfo, bool includeErrors, bool debug)
{
  std::cerr << "Inside GPU RawToDigi, Total pixels: " << wordCounter << " / " << MAX_FED * MAX_WORD * NEVENT << std::endl;
  ...
  std::cerr  << "Raw data is converted into digi for " << NEVENT << " event(s)" << std::endl;
