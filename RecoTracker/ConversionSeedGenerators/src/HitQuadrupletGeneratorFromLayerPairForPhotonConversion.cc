								      ...
								      const edm::Event& event, const edm::EventSetup& es)
{
...
#ifdef mydebug_QSeed
  (*ss) << "In " << innerLayer_.name() << " Out " << theOuterLayer.name() << std::endl;
#endif
  ...
  for (RecHitsSortedInPhi::HitIter oh = outerHits.first; oh!= outerHits.second; ++oh) { 
    ...
    totCountP2++;
    const HitRZCompatibility *checkRZ = region.checkRZ(outerLayer_.detLayer(), ohit, es);
    ...
    if(failCheckRZCompatibility(nohit,*outerLayer_.detLayer(),checkRZ,region)) continue;
    ...
    */
    ...
    const HitRZCompatibility *checkRZb = region.checkRZ(innerLayer_.detLayer(),  ohit, es);
    const HitRZCompatibility *checkRZc = region.checkRZ(innerLayer_.detLayer(), nohit, es);
    ...
    for ( vector<RecHitsSortedInPhi::Hit>::const_iterator ih=innerHits.begin(); ih < ieh; ++ih) {  
      ...
      if(failCheckRZCompatibility(ihit,*innerLayer_.detLayer(),checkRZb,region)
	 || 
	 failCheckRZCompatibility(ihit,*innerLayer_.detLayer(),checkRZc,region) ) continue;
	...
	if(failCheckRZCompatibility(nihit,*innerLayer_.detLayer(),checkRZb,region)
	   || 
	   failCheckRZCompatibility(nihit,*innerLayer_.detLayer(),checkRZc,region) ) continue;
	...
	if (theMaxElement!=0 && result.size() >= theMaxElement){
	  ...
	  delete checkRZc;	  
#ifdef mydebug_QSeed
	  (*ss) << "In " << innerLayer_.name() << " Out " << outerLayer_.name()
		<< "\tP2 " << totCountP2 
		<< "\tM2 " << totCountM2 
		<< "\tP1 " << totCountP1 
		<< "\tM1 " << totCountM1 
