...
SeedingLayerSetsBuilder::LayerSpec::LayerSpec(unsigned short index, const std::string& layerName, const edm::ParameterSet& cfgLayer, edm::ConsumesCollector& iC):
  ...
  useErrorsFromParam(cfgLayer.exists("useErrorsFromParam") ? cfgLayer.getParameter<bool>("useErrorsFromParam") : false)

{
  ...
  if(useErrorsFromParam) {
    hitErrorRPhi = cfgLayer.getParameter<double>("hitErrorRPhi");
    hitErrorRZ   = cfgLayer.getParameter<double>("hitErrorRZ");
  }
...
std::string SeedingLayerSetsBuilder::LayerSpec::print(const std::vector<std::string>& names) const
{
  ...
  str << "Layer="<<names[nameIndex]<<", hitBldr: "<<hitBuilder<<", useErrorsFromParam: ";
  if (useErrorsFromParam) {
