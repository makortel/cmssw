...
namespace {
  ...
  bool phiRangesOverlap(const float x1,const float x2, const float y1,const float y2) {
    ...
    for(const auto& innerSpan: innerSpans) {
      for(const auto& outerSpan: outerSpans) {
        ...
        if(phiRangesOverlap(innerSpan.phiSpan, outerSpan.phiSpan)) {
          ...
          const auto outerDet = std::get<0>((*inactiveLayers_)[layerIdxPair.first]);
...
void PixelInactiveAreaFinder::updatePixelDets(const edm::EventSetup& iSetup) {
  ...
  // deduce the number of ladders per layer and the number of modules per ladder
  {
    ...
    edm::LogWarning("PixelInactiveAreaFinder") << "Number of modules per ladder " << nModulesPerLadder << "; below are number of ladders per layer";

    // number of ladders
    for(unsigned layer=0; layer<4; ++layer) {
      nBPixLadders[layer] = std::count_if(counts[layer].begin(), counts[layer].end(), [](unsigned short val) { return val > 0; });
      edm::LogPrint("PixelInactiveAreaFinder") << "BPix layer " << (layer+1) << " has " << nBPixLadders[layer] << " ladders";
