  ...
  namespace detail {
    void processEDAliases(std::vector<std::string> const& aliasNamesToProcess,
      ...
      for (auto const& aliasEntry : aliasMap) {
        // First check if the alias has already been inserted
        ProductRegistry::ProductList::const_iterator it = preg.productList().find(aliasEntry.second);
        if (it != preg.productList().end()) {
          assert(it->second.isAlias());
          continue;
        }
        // Then check that the alias-for product exists
        it = preg.productList().find(aliasEntry.first);
