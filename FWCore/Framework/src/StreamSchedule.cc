  ...
  void StreamSchedule::fillWorkers(ParameterSet& proc_pset,
    ...
    if (itCondBegin != modnames.end()) {
      ...
      for (auto const& cond : conditionalmods) {
        ...
        //TODO: what if there is an alias to a module explicitly in the Path?
      ...
      //find SwitchProducers whose chosen case is an alias
      {
        ...
        edm::LogPrint l("foo");
        l << " processing EDAliases for";
        for (auto const& a : switchEDAliases) {
          l << " " << a;
        }
        detail::processEDAliases(switchEDAliases, conditionalmods, proc_pset, processConfiguration->processName(), preg);
      }
      {
