    ...
    def __init__(self, accelerators):
        ...
        if args.setInResolver != "":
            if args.setInResolver not in self._variants:
                raise Exception("Trying to set variant globally in TestModuleTypeResolver to {}, but a corresponding accelerator is not available".format(args.setInResolver))
            self._variants.remove(args.setInResolver)
            self._variants.insert(0, args.setInResolver)
        if len(self._variants) == 0:
            raise Exception("No 'cpu' or 'other' accelerator available")

    def pluginAndConfiguration(self):
        return ("edm::test::ConfigurableTestTypeResolverMakerPlugin", cms.untracked.PSet(
            variant = cms.untracked.string(self._variants[0])
        ))
...
class ProcessAcceleratorTest(cms.ProcessAccelerator):
    ...
    def moduleTypeResolver(self, accelerators):
        return TestModuleTypeResolver(accelerators)
