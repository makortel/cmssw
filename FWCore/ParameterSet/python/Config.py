...
#                delattr(self, name)
    ...
    def handleProcessAccelerators(self, parameterSet):
        ...
        if moduleTypeResolver is not None:
            ...
            (moduleTypeResolverPlugin, moduleTypeResolverPSet) = moduleTypeResolver.pluginAndConfiguration()
            if not isinstance(moduleTypeResolverPSet, PSet):
                raise TypeError("{}.pluginAndConfiguration()'s return tuple's second element must be of type cms.untracked.PSet, got {}".format(
                    moduleTypeResolver.__class__.__name__,
                    PSet.__class__.__name__))
            if moduleTypeResolverPSet.isTracked():
                raise TypeError("{}.pluginAndConfiguration()'s return tuple's second element must be untracked, got a tracked PSet".format(moduleTypeResolver.__class__.__name__))
            ...
            for modlist in [self.producers_, self.filters_, self.analyzers_,
                ...
                for module in modlist().values():
                    moduleTypeResolver.setModuleBackend(module)
...
class ProcessAccelerator(_ConfigureComponent,_Unlabelable):
    ...
    def moduleTypeResolver(self, accelerators):
        ...
        - pluginAndConfiguration(self):
          * should return a tuple (plugin_name, PSet)
        - setModuleBackend(self, module):
...
if __name__=="__main__":
    ...
    class TestModuleTypeResolver:
        def __init__(self, accelerators, pset=untracked.PSet()):
            ...
            if len(self._valid_backends) == 0:
                raise Exception("Machine has no accelerators that Test supports (has {})".format(", ".join(accelerators)))
            self._pset = pset

        def pluginAndConfiguration(self):
            return ("TestModuleTypeResolver", self._pset)
                ...
                if hasattr(module, "test"):
                    if hasattr(module.test, "backend"):
                        if module.test.backend.value() not in self._valid_backends:
                            raise Exception("Module {} has the Test backend set explicitly, but its accelerator is not available for the job".format(module.label_()))
                    else:
