...
VectorHit::VectorHit(const VectorHit& vh):
  BaseTrackerRecHit(vh.localPosition(), LocalError(vh.parametersError()[2][2],vh.parametersError()[2][3],vh.parametersError()[3][3]),
...
VectorHit::VectorHit(const GeomDet& idet,
  ...
  BaseTrackerRecHit(posLower, LocalError(covMatrix[2][2],covMatrix[2][3],covMatrix[3][3]),
    idet, trackerHitRTTI::vector),
...
VectorHit::VectorHit(const GeomDet& idet, const VectorHit2D& vh2Dzx, const VectorHit2D& vh2Dzy,
                     OmniClusterRef const& lower, OmniClusterRef const& upper) :
  BaseTrackerRecHit(LocalPoint(vh2Dzx.localPosition().x(), vh2Dzy.localPosition().x(), 0.),
    LocalError(vh2Dzx.covMatrix()[1][1],0.0,vh2Dzy.covMatrix()[1][1]),
    idet, trackerHitRTTI::vector),
  theDimension(4),
  theLowerCluster(lower),
  theUpperCluster(upper)
