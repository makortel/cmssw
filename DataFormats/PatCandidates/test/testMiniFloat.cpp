...
bool testMax32ConvertedToMax16() {
  // max32ConvertedToMax16() -> float16 -> float32 should be the same as max()
  const float max32ConvertedTo16 = MiniFloatConverter::float16to32(MiniFloatConverter::float32to16(MiniFloatConverter::max32ConvertedToMax16()));
  if(max32ConvertedTo16 != MiniFloatConverter::max()) {
    std::cout << "MiniFloatConverter::max32ConvertedToMax16() converted to float16->float32 does not give MiniFloatConverter::max() (" << MiniFloatConverter::max() << "), but " << max32ConvertedTo16 << std::endl;
    return false;
  }

  // max32ConvertedToMax16() + 1ulp should give inf
  union { float flt; uint32_t i32; } conv;
  conv.flt = MiniFloatConverter::max32ConvertedToMax16();
  conv.i32 += 1;
  ...
  if(edm::isFinite(max32PlusConvertedTo16)) {
    std::cout << "MiniFloatConverter::max32ConvertedToMax16() + 1ulp ->float16->float32 does not yield inf but " << max32PlusConvertedTo16 << std::endl;  }
...
int main(void) {
  ...
  success = success && testMax32ConvertedToMax16();
