...
namespace {
  ...
  void addSlimmingChildrenCount(edm::ThinnedAssociationsHelper const& helper,
    ...
    if (slimmingChildren >= 2) {
      // does this case actually ever happen?
      throw edm::Exception(edm::errors::LogicError) << "I don't think 'slimmingChildren >= 2' can ever happen";
      //throw edm::Exception(edm::errors::LogicError) << "Encountered a parent collection with BranchID " << branches.thinned().id() << " that has more than one thinned children that are either themselves slimmed, or have further thinned children that are slimmed. This is not allowed. In the thinning parentage tree, any parent may have slimmed collections in at most one path to any leaf thinned collections of that parent.";
    }
