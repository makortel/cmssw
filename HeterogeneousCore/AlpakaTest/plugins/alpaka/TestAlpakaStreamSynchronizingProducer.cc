...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  public:
    ...
    void acquire(DeviceEvent const& iEvent, DeviceEventSetup const& iSetup) override {
      [[maybe_unused]] auto const& esData = iSetup.getData(esTokenDevice_);

      helper_.makeAsync(iEvent, iSetup);
    }

    void produce(DeviceEvent& iEvent, DeviceEventSetup const& iSetup) override {
  ...
  private:
    const ESDeviceGetToken<AlpakaESTestDataDDevice, AlpakaESTestRecordD> esTokenDevice_;
