...
namespace cms::cudatest {
  inline unsigned long findPrimes(const unsigned n_iterations) {
  ...
  */
  ...
  public:
    ...
    TimeCruncher(const std::vector<unsigned int>& iters, const std::vector<double>& times) {
      if (iters.size() != times.size()) {
        throw cms::Exception("Configuration")
            << "CPU Calibration: got " << iters.size() << " iterations and " << times.size() << " times";
      }
      if (iters.empty()) {
    ...
    void crunch_for(const std::chrono::nanoseconds& crunchtime) const {
      ...
      LogTrace("foo") << "crunch for " << (crunchtime.count() * 1e-3) << " us == " << niters
                      << " iter. actual time: " << (actual.count() * 1e-3)
  ...
  private:
    ...
    std::vector<unsigned int> niters_vect_ = {
        0,    10,    30,    50,    70,    100,   200,   300,   400,   500,   600,   700,  800,
        1000, 1300,  1600,  2000,  2300,  2600,  3000,  3300,  3500,  3900,  4200,  5000, 6000,
        8000, 10000, 12000, 15000, 17000, 20000, 25000, 30000, 35000, 40000, 50000, 60000
        // long calib
  ...
  inline const TimeCruncher& getTimeCruncher() {
