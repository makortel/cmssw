...
TestCUDAProducerSimEWSingle::TestCUDAProducerSimEWSingle(const edm::ParameterSet& iConfig) {
  ...
  for(const auto& moduleLabel: iConfig.getParameter<std::vector<std::string> >("modules")) {
    ...
    if(acquireOpsCPU.events() == 0 && acquireOpsGPU.events() == 0) {
      throw cms::Exception("Configuration") << "Got 0 events for module " << moduleLabel << ", which makes this module useless";
    }
    ...
    if(nevents != produceOpsCPU.events() and produceOpsCPU.events() > 0) {
      throw cms::Exception("Configuration") << "Got " << nevents << " events for acquire and " << produceOpsCPU.events() << " for produce CPU for module " << moduleLabel;
    }
    if(nevents != produceOpsGPU.events() and produceOpsGPU.events() > 0) {
      throw cms::Exception("Configuration") << "Got " << nevents << " events for acquire and " << produceOpsGPU.events() << " for produce GPU for module " << moduleLabel;
    }

    modules_.emplace_back(std::move(acquireOpsCPU),
                          std::move(acquireOpsGPU),
                          std::move(produceOpsCPU),
                          std::move(produceOpsGPU));
  }
}
