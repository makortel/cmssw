  ...
  gangNum_{iConfig.getParameter<unsigned int>("gangNumber")}
{
  ...
  if(cs.isAvailable() and cs->enabled()) {
    boost::property_tree::ptree calib_root_node;
    boost::property_tree::read_json(iConfig.getParameter<edm::FileInPath>("cudaCalibration").fullPath(), calib_root_node);
    std::vector<unsigned int> iters;
    std::vector<double> times;
    for(const auto& elem: calib_root_node.get_child("niters")) {
      iters.push_back(elem.second.get<unsigned int>(""));
    ...
    for(const auto& modfunc: module.second) {
      ...
      if(not opsCPU.empty()) {
        ...
        for(const auto& op: opsCPU) {
          if(op->size() != nevents) {
            throw cms::Exception("Configuration") << "Inconsistency in number of events in CPU operations of module " << moduleName << " " << op->size() << " != " << nevents;
          }
        }
      }
      if(not opsGPU.empty()) {
        ...
        for(const auto& op: opsGPU) {
          if(op->size() != nevents) {
            throw cms::Exception("Configuration") << "Inconsistency in number of events in GPU operations of module " << moduleName << " " << op->size() << " != " << nevents;
          }
        }
      }
...
SimOperationsService::GPUData::GPUData(GPUData&& rhs) {
  swap(rhs);
}
...
namespace {
  ...
  std::tuple<int, size_t> indexEvents(const T& ops, const std::string& moduleLabel) {
    int index = -1;
    size_t nevents = 0;
    const auto found = std::find_if(ops.begin(), ops.end(), [&](const auto& elem) {
        return elem.first == moduleLabel;
      });
    if(found != ops.end()) {
...
SimOperationsService::AcquireCPUProcessor SimOperationsService::acquireCPUProcessor(const std::string& moduleLabel) const {
...
SimOperationsService::ProduceCPUProcessor SimOperationsService::produceCPUProcessor(const std::string& moduleLabel) const {
  const auto ie = indexEvents(produceOpsCPU_, moduleLabel);
  return ProduceCPUProcessor(std::get<0>(ie), this, std::get<1>(ie));
}
SimOperationsService::ProduceGPUProcessor SimOperationsService::produceGPUProcessor(const std::string& moduleLabel) const {
  const auto ie = indexEvents(produceOpsGPU_, moduleLabel);
  if(std::get<0>(ie) >= 0) {
    return ProduceGPUProcessor(std::get<0>(ie), this, GPUData(produceOpsGPU_.at(std::get<0>(ie)).second, gangSize_), std::get<1>(ie));
  }
