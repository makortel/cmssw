...
namespace {
  ...
  public:
    ...
    void enqueue(unsigned int eventIndex, std::vector<const cms::cuda::Product<int>*> inputData, edm::WaitingTaskWithArenaHolder& holder, cms::cuda::ContextState* ctxState) const {
      ...
      if(queueTask) {
        taskQueue->push([this, ctxState]() mutable {
...
class TestCUDAProducerSimEWGangedLimitedTaskQueueV4: public edm::stream::EDProducer<edm::ExternalWork, edm::GlobalCache<Ganger>> {
public:
  explicit TestCUDAProducerSimEWGangedLimitedTaskQueueV4(const edm::ParameterSet& iConfig, const Ganger* ganger);
...
TestCUDAProducerSimEWGangedLimitedTaskQueueV4::TestCUDAProducerSimEWGangedLimitedTaskQueueV4(const edm::ParameterSet& iConfig, const Ganger* ganger) {
...
void TestCUDAProducerSimEWGangedLimitedTaskQueueV4::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void TestCUDAProducerSimEWGangedLimitedTaskQueueV4::acquire(const edm::Event& iEvent, const edm::EventSetup& iSetup, edm::WaitingTaskWithArenaHolder h) {
  //edm::LogWarning("foo") << "TestCUDAProducerSimEWGangedLimitedTaskQueueV4::acquire() begin";
...

  {
    ...
    auto ctx = cudaProducts.empty() ? cms::cuda::ScopedContextAcquire(iEvent.streamID(), ctxState_) :
      cms::cuda::ScopedContextAcquire(*cudaProducts[0], ctxState_);
  }
  ...
  //edm::LogWarning("foo") << "TestCUDAProducerSimEWGangedLimitedTaskQueueV4::acquire() end";
}

void TestCUDAProducerSimEWGangedLimitedTaskQueueV4::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  //edm::LogWarning("foo") << "TestCUDAProducerSimEWGangedLimitedTaskQueueV4::produce()";
...
DEFINE_FWK_MODULE(TestCUDAProducerSimEWGangedLimitedTaskQueueV4);
