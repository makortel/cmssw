...
class TestCUDAProducerSimEWGangedV2: public edm::stream::EDProducer<edm::ExternalWork, edm::GlobalCache<Ganger>> {
public:
  explicit TestCUDAProducerSimEWGangedV2(const edm::ParameterSet& iConfig, const Ganger* ganger);
...
TestCUDAProducerSimEWGangedV2::TestCUDAProducerSimEWGangedV2(const edm::ParameterSet& iConfig, const Ganger* ganger) {
  const auto moduleLabel = iConfig.getParameter<std::string>("@module_label");
  edm::Service<SimOperationsService> sos;
...
void TestCUDAProducerSimEWGangedV2::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void TestCUDAProducerSimEWGangedV2::acquire(const edm::Event& iEvent, const edm::EventSetup& iSetup, edm::WaitingTaskWithArenaHolder h) {
...
DEFINE_FWK_MODULE(TestCUDAProducerSimEWGangedV2);
