...
namespace cms::cudatest {
  GPUTimeCruncher::GPUTimeCruncher(const std::vector<unsigned int>& iters, const std::vector<double>& times) {
    if (iters.size() != times.size()) {
      throw cms::Exception("Configuration")
          << "CUDA Calibration: got " << iters.size() << " iterations and " << times.size() << " times";
    }
    if (iters.empty()) {
    ...
    float* kernel_data_d;
    auto h_src =
        cms::cuda::make_host_noncached_unique<char[]>(kernel_elements * sizeof(float) /*, cudaHostAllocWriteCombined*/);
  ...
  void GPUTimeCruncher::crunch_for(const std::chrono::nanoseconds& time,
                                   float* kernel_data_d,
