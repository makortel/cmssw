...
TestCUDAAnalyzerGPUKernel::TestCUDAAnalyzerGPUKernel(cuda::stream_t<>& stream) {
  ...
  sum_ = cs->make_device_unique<float[]>(NUM_VALUES, stream);
...
float TestCUDAAnalyzerGPUKernel::value(cuda::stream_t<>& stream) const {
  edm::Service<CUDAService> cs;
  auto accumulator = cs->make_device_unique<float>(stream);
  auto h_accumulator = cs->make_host_unique<float>(stream);
