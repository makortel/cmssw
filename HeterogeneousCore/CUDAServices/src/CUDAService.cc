...
namespace {
  ...
  void devicePreallocate(CUDAService& cs, int numberOfDevices, const std::vector<unsigned int>& bufferSizes) {
    ...
    for (int i = 0; i < numberOfDevices; ++i) {
      ...
      preallocate<cudautils::device::unique_ptr>(
          [&](size_t size, cuda::stream_t<>& stream) { return cs.make_device_unique<char[]>(size, stream); },
          bufferSizes);
    }
  ...
  void hostPreallocate(CUDAService& cs, const std::vector<unsigned int>& bufferSizes) {
    preallocate<cudautils::host::unique_ptr>(
        [&](size_t size, cuda::stream_t<>& stream) { return cs.make_host_unique<char[]>(size, stream); }, bufferSizes);
  }
}  // namespace
...
CUDAService::CUDAService(edm::ParameterSet const& config, edm::ActivityRegistry& iRegistry) {
  ...
  devicePreallocate(
      *this, numberOfDevices_, allocator.getUntrackedParameter<std::vector<unsigned int>>("devicePreallocate"));
...
void CUDAService::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  allocator.addUntracked<bool>("enabled", true)
      ->setComment("Enable caching. Setting this to false means to call cudaMalloc for each allocation etc.");
  allocator.addUntracked<unsigned int>("binGrowth", 8)
      ->setComment("Growth factor (bin_growth in cub::CachingDeviceAllocator");
  allocator.addUntracked<unsigned int>("minBin", 1)
      ->setComment("Smallest bin, corresponds to binGrowth^minBin bytes (min_bin in cub::CacingDeviceAllocator");
  allocator.addUntracked<unsigned int>("maxBin", 9)
      ->setComment(
          "Largest bin, corresponds to binGrowth^maxBin bytes (max_bin in cub::CachingDeviceAllocator). Note that "
          "unlike in cub, allocations larger than binGrowth^maxBin are set to fail.");
  allocator.addUntracked<unsigned int>("maxCachedBytes", 0)
      ->setComment("Total storage for the allocator. 0 means no limit.");
  allocator.addUntracked<double>("maxCachedFraction", 0.8)
      ->setComment(
          "Fraction of total device memory taken for the allocator. In case there are multiple devices with different "
          "amounts of memory, the smallest of them is taken. If maxCachedBytes is non-zero, the smallest of them is "
          "taken.");
  ...
  desc.addUntracked<edm::ParameterSetDescription>("allocator", allocator)
      ->setComment("See the documentation of cub::CachingDeviceAllocator for more details.");
...
struct CUDAService::Allocator {
  ...
  Allocator(size_t max, Args&&... args)
      : maxAllocation(max), deviceAllocator(args...), hostAllocator(std::forward<Args>(args)...) {}
...
void* CUDAService::allocate_device(int dev, size_t nbytes, cuda::stream_t<>& stream) {
  ...
  if (allocator_) {
    if (nbytes > allocator_->maxAllocation) {
      throw std::runtime_error("Tried to allocate " + std::to_string(nbytes) + " bytes, but the allocator maximum is " +
                               std::to_string(allocator_->maxAllocation));
    }
...
std::shared_ptr<cuda::event_t> CUDAService::getCUDAEvent() {
  return cudaEventCache_->cache[getCurrentDevice()].makeOrGet([]() {
    ...
    return std::make_unique<cuda::event_t>(current_device.create_event(
        cuda::event::
